...
import { EVMWalletClient } from "../core/EVMWalletClient";
...
export class BalancerService {
    ...
    async swapOnBalancer(walletClient: EVMWalletClient, parameters: SwapParameters) {
        ...
        const allowance = await this.sdk.crypto.getAllowance({
            wallet: {
        ...
        const tx = await this.sdk.crypto.sendTxn({
            wallet: {

            },
            payload: {
                toAddress: callData.to as `0x${string}`,
                txData: callData.callData,
            },
          }); 
    ...
    async addLiquidity(walletClient: EVMWalletClient, parameters: LiquidityParameters) {
        ...
        const tx = await this.sdk.crypto.sendTxn({
            ...
            payload: {
                toAddress: call.to as `0x${string}`,
                txData: call.callData,
            },
          }); 
        return {
            success: true,
